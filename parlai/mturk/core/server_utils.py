...
def setup_heroku_server(task_files_to_copy=None):
    # Check if Heroku CLI exists
    if not sh.which('heroku'):
        # Install Heroku CLI
        os_name = None
        ...
        sh.contrib.sudo.mkdir(shlex.split('-p /usr/local/lib /usr/local/bin'))
        sh.contrib.sudo.mv(shlex.split(os.path.join(parent_dir, 'heroku-cli-*')+' /usr/local/lib/heroku'))
        sh.contrib.sudo.ln(shlex.split('-s /usr/local/lib/heroku/bin/heroku /usr/local/bin/heroku'))

        os.chdir(parent_dir)
        sh.rm(shlex.split('-rf heroku-cli-*/'))
        sh.rm(shlex.split('heroku.tar.gz'))
    ...
    hit_config_file_path = os.path.join(parent_dir, 'hit_config.json')
    shutil.copy2(hit_config_file_path, task_directory_path)

    server_config_file_path = os.path.join(parent_dir, 'server_config.json')
    shutil.copy2(server_config_file_path, heroku_server_directory_path)
    ...
    while not heroku_user_identifier:
        try:
            ...
            raise SystemExit("A free Heroku account is required for launching MTurk tasks. Please register at https://signup.heroku.com/ and then run `heroku login` at the terminal to login to Heroku first before running this program.")

    heroku_app_name = (user_name + '-' + hashlib.md5(heroku_user_identifier.encode('utf-8')).hexdigest())[:30]
    try:
        sh.heroku(shlex.split('create ' + heroku_app_name))
    except sh.ErrorReturnCode_1:
        sh.heroku(shlex.split('git:remote -a ' + heroku_app_name))

    print("Heroku: Starting server...")

    # Enable WebSockets
    try:
        sh.heroku(shlex.split('features:enable http-session-affinity'))
    except sh.ErrorReturnCode_1:
        # Already enabled WebSockets
        pass

    # os.system('git add -A && git commit -m "app" && git push -f heroku master')
    os.chdir(heroku_server_directory_path)
